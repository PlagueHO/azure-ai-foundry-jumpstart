...
def test_init_with_explicit_params() -> None:
...
def test_init_with_env_vars(mock_env_vars) -> None:
...
def test_init_missing_required_vars() -> None:
...
def test_create_kernel(mock_kernel) -> None:
...
def test_create_kernel_auth_methods(api_key, mock_kernel) -> None:
...
def test_create_prompt_function(mock_kernel) -> None:
...
@pytest.mark.asyncio
async def test_run(mock_kernel, temp_output_dir) -> None:
    """Test the run method."""
    tool = SimpleTestTool()
...
@pytest.mark.asyncio
async def test_run_async(mock_kernel, temp_output_dir) -> None:
...
def test_generate_data(mock_kernel, temp_output_dir) -> None:
    ...
    with patch("semantic_kernel.Kernel", return_value=mock_kernel):
        ...
        with patch.object(generator, "run") as mock_run:
            ...
            generator.generate_data(
                count=10, out_dir=temp_output_dir, output_format="json"
            )
...
def test_persist_json(temp_output_dir) -> None:
...
def test_persist_yaml(temp_output_dir) -> None:
...
def test_persist_txt(temp_output_dir) -> None:
